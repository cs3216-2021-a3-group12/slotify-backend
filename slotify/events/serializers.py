from rest_framework import serializers

from .models import Event
from .constants import EVENT_MAX_DESCRIPTION_LENGTH, EVENT_MAX_TITLE_LENGTH, EVENT_MIN_TITLE_LENGTH, EVENT_MAX_LOCATION_LENGTH

class DateTimeRangeSerializer(serializers.Serializer):
    start_date_time = serializers.IntegerField(min_value=0)
    end_date_time = serializers.IntegerField(min_value=0)

    def validate(self, data):
        """
        Check that start_date_time is before end_date_time.
        """
        if data["start_date_time"] >= data["end_date_time"]:
            raise serializers.ValidationError(
                "start date/time must be before end date/time"
            )

        return data


class PostEventSerializer(serializers.ModelSerializer):
    title=serializers.CharField(max_length=EVENT_MAX_TITLE_LENGTH, min_length=EVENT_MIN_TITLE_LENGTH)
    description=serializers.CharField(max_length=EVENT_MAX_DESCRIPTION_LENGTH)
    location = serializers.CharField(max_length=EVENT_MAX_LOCATION_LENGTH)
    isPublic = serializers.BooleanField(default=True)
    date_time_ranges = DateTimeRangeSerializer(many=True, allow_empty=False)

    class Meta:
        model = Event
        fields = ["title", "description", "date_time_ranges", "location", "is_public"]